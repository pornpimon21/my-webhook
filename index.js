...
// =================================================================
// 🛡️ ส่วนรับชื่อ (get name) - ปรับปรุงใหม่เพื่อลดการเด้งซ้ำ
// =================================================================
if (intent === "get name") {
    const name = params.name || "คุณ";
    session.name = name;
    await saveSession(session); // บันทึกชื่อลง MongoDB
    ...
    // เตรียมปุ่มกด ( levelBubbles ที่คุณสงสัยว่าอยู่ไหน )
    ...
    // ✅ มัดรวมส่งกลับทีเดียวผ่าน res.json (ตัด pushMessage และ setTimeout ทิ้ง)
    return res.json({
        fulfillmentMessages: [
            {
                text: { 
            ...
            },
            {
                payload: {
                    line: {
                        ...
                        contents: {
                            type: "carousel",
                            contents: levelBubbles // ดึงค่าปุ่มที่เตรียมไว้มาใส่ตรงนี้
                        }
                    }
                }
            }
