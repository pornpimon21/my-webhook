...
// ฟังก์ชันเปรียบเทียบความใกล้เคียง (ฉบับอัปเกรด AI)
function findClosestAbility(userInput, similarityThreshold = 0.55) { // 🟢 ปรับ Threshold ลงเพื่อให้รับคำผิดได้ง่ายขึ้น
  // 1. แปลงข้อความเป็นตัวพิมพ์เล็ก และตัดช่องว่างหน้าหลัง
  ...
  // 🎯 2. Exact Match: ถ้าพิมพ์ถูกเป๊ะๆ เอาเลย
  if (allAbilities.includes(userInput)) return userInput;

  // 🔍 3. Smart Searching: วนลูปหาคำที่ดีที่สุด
  let bestMatch = null;
  let highestScore = 0;
  ...
  for (const ability of allAbilities) {
    ...
    // A. กรณี "พิมพ์ตก" หรือ "พิมพ์ไม่ครบ" (เช่น "ฟิสิก" -> "ฟิสิกส์")
    // ถ้าคำในระบบ "ขึ้นต้นด้วย" หรือ "ประกอบด้วย" สิ่งที่ user พิมพ์มา
    if (ability.includes(userInput) && userInput.length >= 3) {
      // ให้คะแนนสูงมาก (0.8 - 0.99) ยิ่งพิมพ์มาครบยิ่งได้คะแนนเยอะ
      score = 0.8 + (userInput.length / ability.length) * 0.1;
    } 
    // B. กรณี "พิมพ์ผิด" (เช่น "คมี" -> "เคมี") ใช้ Levenshtein วัดระยะห่าง
    else {
      const dist = levenshtein.get(userInput, ability);
      const maxLength = Math.max(userInput.length, ability.length);
      
      // สูตรคำนวณความเหมือน (0 ถึง 1)
      score = 1 - (dist / maxLength);
    }

    // เก็บค่าที่ดีที่สุดไว้
  ...
  // 🟢 เช็คคะแนน: ถ้าผ่านเกณฑ์ หรือเป็นคำสั้นๆ ที่ผิดนิดเดียว ให้ผ่านเลย
  // ช่วยกรณีคำสั้นๆ เช่น "คมี" (3 ตัว) ผิด 1 ตัว จะโดนหักคะแนนเยอะ เราจึงต้องช่วยดันให้ผ่าน
  if (highestScore >= similarityThreshold) {
    return bestMatch;
  }

  return null; // ถ้าไม่ใกล้เคียงเลยจริงๆ ให้คืนค่า null
}
...
let invalid = [];
    ...
    // 🔴 ลบอันเก่าที่เช็ค invalid.length > 0 ออกไปเลยครับ
    // ✅ ใช้อันนี้แทน: เช็คว่า "ถ้าหาคำที่ถูกต้องไม่เจอเลยสักคำ" ค่อย Error
    ...
    // (ส่วนข้างล่างทำงานต่อได้เลย โดยใช้เฉพาะคำที่ Valid)
