...
if (userMessage === 'ค้นหาความถนัด') {
  ...
  const question = buildQuestionFlex(0); // คาดว่าเป็น array
  ...
  await client.replyMessage(event.replyToken, [
    ...
    ...question // กระจาย array ถ้า question เป็น array
  ]);
  return;
}
...
if (userSessions[userId]) {
  ...
  if (session.step < questions.length) {
    ...
    // ส่งแบบ array หรือ ถ้า nextQuestion เป็น object เดี่ยว ให้ใส่ใน array
    ...
    const resultText =
      `🎯 ผลการวิเคราะห์ความถนัดของคุณ\n\n` +
      `📌 คุณเหมาะกับ\n➡️ สายงาน : ${result.bestTrack}\n\n` + // ใช้ bestTrack ตาม analyzeAnswers ตัวอย่างก่อนหน้า
      `💡  จุดเด่นของคุณ\n${result.traits.join(', ')}\n\n` +
          `คำแนะนำ:\nลองมุ่งเน้นพัฒนาทักษะที่เกี่ยวข้องกับสายงานนี้เพื่อความก้าวหน้าในอนาคต\n\n` +
