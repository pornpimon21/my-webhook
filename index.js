    ...
    try {
      ...
      await Promise.all(events.map(async (event) => {
        ...
        if (event.type === 'message' && event.message.type === 'text') {
          ...
          // --- [ ส่วนที่ต้องเพิ่มเข้าไปใหม่ ] ---
  // สร้างรายการคำสำคัญที่เกี่ยวข้องกับระบบของคุณ (MongoDB/Dialogflow)
  const myKeywords = [
    "แนะนำคณะ", "คณะ", "สาขา", "ความถนัด", "เกรด", "เริ่ม", "ค้นหา", 
    "คำถามที่พบบ่อย", "คำกิจกรรมที่ระบบเข้าใจ", "แผนการเรียน"
  ];

  // ตรวจสอบว่าข้อความที่ลูกค้าพิมพ์มา มีคำในรายการของเราหรือไม่
  const isMyJob = myKeywords.some(keyword => userMessage.includes(keyword)) || 
                   userSessions[userId]; // หรือถ้า User กำลังทำ Flow ค้างไว้

  if (!isMyJob) {
    // 🛑 ถ้าไม่ใช่เรื่องที่ระบบเราดูแล ให้ "จบการทำงาน" ทันทีโดยห้ามสั่ง replyMessage
    // เพื่อให้ Reply Token เหลือไปถึง AI Beta ของ LINE OA
    console.log("⏩ ปล่อยให้ AI Beta ของ LINE ตอบเรื่องทั่วไป:", userMessage);
   
    // ✅ ต้องมีบรรทัดนี้ เพื่อบอก LINE ว่า "รับทราบแล้ว แต่ฉัน (Webhook) จะไม่ตอบนะ"
    // เพื่อให้ระบบ LINE ไปดึง AI Chatbot Beta มาทำงานแทน
