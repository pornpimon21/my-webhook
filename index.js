...
if (userMessage === 'ค้นหาความถนัด') {
    ...
    type: 'text',
    text: 'สวัสดีค่ะ!\nวันนี้เราจะช่วยคุณค้นหาความถนัดที่เหมาะสมกับตัวเองผ่านคำถามง่าย ๆ ไม่กี่ข้อ\nกรุณาเลือกคำตอบที่ตรงกับตัวคุณมากที่สุด เพื่อผลลัพธ์ที่แม่นยำที่สุดนะคะ 😊\nพร้อมแล้วไปเริ่มกันเลย!'
  });
...
if (userMessage === 'เริ่มแนะนำใหม่') {
  ...
  if (session && session.name && session.grade && session.educationLevel) {
    // รีเซตค่าความถนัดและคำแนะนำ
    session.abilitiesInputText = '';
    session.recommendations = [];

    // เซฟ session ที่แก้ไขแล้ว
    await session.save();
