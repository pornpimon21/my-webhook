...
if (intent === "get name") {
    ...
    // 2. ใช้ฟังก์ชันเทพของเรา ตรวจสอบว่ามันคือทักษะหรือไม่?
    const detectedSkill = findClosestAbility(userInput); // จะได้ค่า "คณิต" กลับมา

    // 3. ตรวจสอบยืนยันกับฐานข้อมูลจริง (กันเหนียว)
    const allValidSkills = faculties.flatMap(f => f.majors.flatMap(m => m.ability));
    
    // เช็คว่า คำที่แก้แล้ว (detectedSkill) หรือคำเดิม มีอยู่ใน DB ไหม
    // และต้องมีความยาวมากกว่า 2 ตัวอักษร (กันพวกคำสั้นๆ ที่อาจเป็นชื่อเล่น)
    const isSkill = allValidSkills.some(s => s === detectedSkill || s === userInput) && userInput.length > 2;

    let replyText = "";

    if (isSkill) {
      // ✅ กรณี: ผู้ใช้พิมพ์ทักษะมา (บอทรู้ทัน!)
      console.log(`💡 ผู้ใช้พิมพ์ทักษะ "${userInput}" แทนชื่อ -> ระบบแก้เป็น "${detectedSkill}"`);

      session.name = "นักเรียน"; // ตั้งชื่อสมมติให้
      
      // บันทึกทักษะลง session ไปเลย (ไม่ต้องรอถามทีหลัง)
      if (!session.abilities) session.abilities = [];
      session.abilities.push(detectedSkill);
      
      replyText = `โอ้! สนใจด้าน "${detectedSkill}" สินะคะ (ระบบบันทึกให้แล้วค่ะ ✍️)\n\nงั้นขอข้ามมาถามระดับการศึกษาเลยนะคะ 🎓\n👇 เลือกจากปุ่มด้านล่างได้เลยค่ะ`;

    } else {
      // 👤 กรณี: เป็นชื่อคนจริงๆ (หาไม่เจอใน DB ทักษะ)
      const name = params.name || userInput;
      session.name = name;
      replyText = `👋 สวัสดีค่ะ คุณ${name}\n📘 กรุณาเลือกระดับการศึกษาของคุณ 🎓\n👇 เลือกจากปุ่มด้านล่างได้เลยค่ะ`;
    }

    // บันทึก Session
    await saveSession(session);

    // --- ส่วนสร้างปุ่ม (Flex Message) เหมือนเดิม ---
    ...
    // 4. ส่งข้อความตอบกลับ (ตามเงื่อนไข isSkill)
    res.json({
      fulfillmentText: replyText // ใช้ข้อความที่เราเตรียมไว้ข้างบน
    });

    // 5. Push Message ปุ่มตัวเลือกตามหลัง
    setTimeout(() => {
